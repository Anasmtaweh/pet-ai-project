...
function PetForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(weight) || weight <= 0) {
      setError('Invalid weight (must be greater than 0).');
      return;
    }
    if (!gender) {
    ...
    if (isOtherBreed && breed.trim() === '') {
      // If 'Other' is selected, ensure the custom breed field is filled.
  ...
  const handleSpeciesClick = (selectedSpecies) => {
    if (species !== selectedSpecies) {
      // Only update if species actually changes.
  ...
  return (
    <Container className={`${styles.petFormContainer} mt-5`}>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        {/* Submit Button */}
        <Button className={styles.petFormButton} variant="primary" type="submit">
          Add Pet
        </Button>
      </Form>
    </Container>
