  ...
  while (current.isSameOrBefore(loopEndDate)) {
    ...
    if (rule.repeat) {
      if (rule.repeatType === 'daily') {
        ...
        Array.isArray(rule.repeatDays) &&
        rule.repeatDays.includes(current.format('dddd'))
      ) {
        occursOnThisDay = true;
      }
    } else {
...
function Scheduler() {
  ...
  const handleAddEditModalShow = (ruleToEdit = null) => {
    ...
    if (!ruleToEdit) {
      ...
      setNewEvent({
        ...
        start: ruleToEdit.start
          ? moment(ruleToEdit.start)
  ...
  const handleDeleteConfirmModalShow = (eventOrRule) => {
    ...
    if (eventOrRule.originalRuleId && eventOrRule.start) {
      // Clicked a specific calendar occurrence.
      setSelectedOccurrence(eventOrRule);
    } else {
      // Clicked delete on a rule from the list.
      setSelectedOccurrence(null);
    }
    setShowDeleteConfirmModal(true);
  };
  ...
  const validateEvent = () => {
    ...
    if (newEvent.repeat) {
      // Validation for repeating events.
      const startTime = moment().set({
        hour: newEvent.start.hour(),
        minute: newEvent.start.minute(),
      });
      ...
      // Validation for non-repeating events.
  ...
  const handleAddEvent = async () => {
    ...
    const payload = {
      ...
      ...(newEvent.repeat &&
        newEvent.repeatType === 'weekly' && { repeatDays: newEvent.repeatDays }),
      owner,
    };
    try {
  ...
  const handleDeleteSeries = async () => {
    if (!editEventRule || !editEventRule._id) {
      setDeleteError('Cannot delete series: No rule selected.');
      return;
    }
    setDeleteError('');
    try {
  ...
  const handleDeleteOccurrence = async () => {
    if (!selectedOccurrence || !selectedOccurrence.originalRuleId || !selectedOccurrence.start) {
      setDeleteError('Cannot delete occurrence: Invalid event data selected.');
      return;
    }
    setDeleteError('');
    ...
    try {
      await axios.post(
        `https://mishtika.duckdns.org/schedules/${ruleId}/exception`,
  ...
  const eventPropGetter = (event) => {
    ...
    switch (event.type) {
      case 'meal':
        className = styles.eventMeal;
        break;
      case 'vet':
        className = styles.eventVet;
        break;
      case 'play':
        className = styles.eventPlay;
        break;
      case 'sleep':
        className = styles.eventSleep;
        break;
      case 'medication':
        className = styles.eventMedication;
        break;
      default:
        className = styles.eventDefault;
    }
    return { className };
  };
  ...
  return (
    <Container className={`${styles.schedulerContainer} mt-5`}>
      ...
      <Button
        variant="primary"
        onClick={() => handleAddEditModalShow(null)}
        ...
        {eventRules.length > 0 ? (
          eventRules.map((rule) => (
            <li key={rule._id} className={styles.eventListItem}>
              ...
              {rule.repeat &&
                ` (Repeats ${rule.repeatType}${rule.repeatType === 'weekly' ? ` on ${rule.repeatDays.join(', ')}` : ''})`}
              <br />
              <span className={styles.eventTime}>
                Range: {moment(rule.start).format('MMM Do YYYY')} to{' '}
                {moment(rule.end).format('MMM Do YYYY')} <br />
                Time: {moment(rule.start).format('h:mm a')} - {moment(rule.end).format('h:mm a')}
              </span>
              <div className={styles.eventButtons}>
                <Button variant="primary" size="sm" onClick={() => handleAddEditModalShow(rule)}>
                  Edit Rule
                </Button>
                <Button
                  variant="danger"
                  size="sm"
                  className="ms-2"
                  onClick={() => handleDeleteConfirmModalShow(rule)}
                >
                  Delete
                </Button>
              </div>
      ...
      <Modal show={showAddEditModal} onHide={handleAddEditModalClose}>
          ...
          <Form>
            ...
            <Form.Group className="mb-3" controlId="formEventStart">
              ...
              <Form.Control
                ...
                name="start"
                id="formEventStart"
                value={newEvent.start.format('YYYY-MM-DDTHH:mm')}
            ...
            <Form.Group className="mb-3" controlId="formEventEnd">
              ...
              <Form.Control
                ...
                name="end"
                id="formEventEnd"
                value={newEvent.end.format('YYYY-MM-DDTHH:mm')}
                onChange={(e) => handleDateChange(e, 'end')}
                required
              />
              <Form.Text muted>
                For repeating events, the date part defines the range, and the time part defines the
                daily duration.
              </Form.Text>
            </Form.Group>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={handleAddEditModalClose}>
            Close
          </Button>
        ...
        <Modal.Body>
          ...
          {selectedOccurrence ? (
            <>
              ...
              <p>
                <strong>"{editEventRule?.title}"</strong> on{' '}
                <strong>{moment(selectedOccurrence.start).format('MMM Do YYYY, h:mm a')}</strong>?
              </p>
            </>
          ) : (
            <p>
              Are you sure you want to delete the entire series for{' '}
              <strong>"{editEventRule?.title}"</strong>?
            </p>
          )}
        </Modal.Body>
