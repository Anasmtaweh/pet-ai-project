...
function EditPet() {
    ...
    const fetchPet = async () => {
      ...
      try {
        ...
        setCurrentPictureUrl(
          petData.pictures && petData.pictures.length > 0 ? petData.pictures[0] : '',
        );
        ...
        const isFetchedBreedCustom =
          fetchedBreed && !knownBreedsList.slice(0, -1).includes(fetchedBreed);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(numAgeYears) || numAgeYears < 0 || numAgeYears > 50) {
      setError('Valid age in years (0-50) is required.');
      return;
    }
    if (isNaN(numAgeMonths) || numAgeMonths < 0 || numAgeMonths > 11) {
      setError('Valid age in months (0-11) is required.');
      return;
    }
    if (isNaN(numWeight) || numWeight <= 0 || numWeight > 200) {
      setError('Valid weight (0.1-200 kg) is required.');
      return;
    }
    if (!gender) {
    ...
    if (!breed.trim()) {
      ...
      return;
    } // `breed` holds custom or standard name.
    // --- End Validations ---
    ...
    try {
      ...
      if (err.response?.data?.message) {
        ...
        // For Mongoose validation errors from backend.
        const details = err.response.data.details;
        errorMsg = Object.values(details)
  ...
  const handleSpeciesClick = (selectedSpecies) => {
    if (species !== selectedSpecies) {
      // Only update if species is changing.
