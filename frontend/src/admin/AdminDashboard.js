...
function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const response = await axios.get('https://mishtika.duckdns.org/admin/dashboard', {
          headers: {
      ...
      <table className={styles.recentActivityTable}>
        ...
        <tbody>
          {dashboardData.recentActivity.map((activity, index) => (
            <tr key={index}>
              <td>
                <span className={styles.recentActivityType}>{activity.type}</span>
              </td>
              <td>{activity.details}</td>
              <td>
                <span className={styles.recentActivityTimestamp}>
