...
function AdminUserManagement() {
  ...
  const handleSetStatus = async (userId, desiredStatus) => {
    // Accepts the user ID and the desired boolean status.
    try {
      // API call to update the user's status.
      // Sends the desired status directly to the backend.
      await axios.put(
        `https://mishtika.duckdns.org/admin/users/${userId}`,
      ...
      setUsers(
        users.map((user) => (user._id === userId ? { ...user, isActive: desiredStatus } : user)),
      );
    } catch (err) {
