...
if (!DB_URL) {
  console.error(
    'FATAL ERROR: DB_URL is not defined in environment variables. Please set it in your .env file or environment.',
  );
...
if (!JWT_SECRET) {
  ...
  console.error(
    'FATAL ERROR: JWT_SECRET is not defined in environment variables. Please set it in your .env file or environment.',
  );
...
if (!EMAIL_USER || !EMAIL_PASS) {
  console.warn(
    'WARNING: EMAIL_USER or EMAIL_PASS is not set. Email functionality will be disabled.',
  );
}
if (!OPENAI_API_KEY) {
...
if (!AWS_ACCESS_KEY_ID || !AWS_SECRET_ACCESS_KEY || !AWS_REGION || !S3_BUCKET_NAME) {
  console.warn(
    'WARNING: One or more AWS configuration variables (ACCESS_KEY_ID, SECRET_ACCESS_KEY, REGION, S3_BUCKET_NAME) are missing. S3 related features (e.g., pet image uploads) might not work.',
  );
}
