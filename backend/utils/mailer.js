...
const sendEmail = async (mailOptions) => {
  ...
  if (!transporter) {
    console.error(
      `Cannot send email ("${mailOptions.subject}" to ${mailOptions.to}): Email transporter is not configured.`,
    );
  ...
  try {
    ...
    console.log(
      `Email sent successfully ("${optionsWithDefaults.subject}" to ${optionsWithDefaults.to}): ${info.messageId}`,
    );
    ...
    console.error(
      `Failed to send email ("${optionsWithDefaults.subject}" to ${optionsWithDefaults.to}):`,
      error,
    );
...
const sendReminderEmail = async (toEmail, eventTitle, eventStartTime) => {
  ...
  const formattedStartTime = moment(eventStartTime)
    .tz(reminderTimezone)
