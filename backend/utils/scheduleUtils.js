...
const generateOccurrencesInRange = (rule, windowStart, windowEnd) => {
  ...
  if (!rule || !rule.start || !rule.end || !rule._id || !rule.ownerId) {
    console.warn(
      'BE: Skipping occurrence generation for invalid/incomplete rule (missing required fields like start, end, _id, ownerId):',
      rule?._id,
    );
    return occurrences;
  }
    ...
    while (currentDay.isSameOrBefore(loopEndDate)) {
      ...
      if (rule.repeat) {
        if (rule.repeatType === 'daily') {
          ...
          rule.repeatDays.includes(currentDay.format('dddd'))
        ) {
          occursOnThisDay = true;
        }
      } else {
      ...
      if (occursOnThisDay) {
        ...
        if (
          occurrenceStart.isBetween(windowStartMoment, windowEndMoment, undefined, '[)') &&
