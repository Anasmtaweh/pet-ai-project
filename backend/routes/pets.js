...
router.post('/add', (req, res, next) => {
  ...
  upload.single('picture')(req, res, async (err) => {
    ...
    if (err) {
      if (err instanceof multer.MulterError) {
        if (err.code === 'LIMIT_FILE_SIZE') {
          return res
            .status(400)
            .json({ message: `File too large. Maximum size is ${MAX_FILE_SIZE / 1024 / 1024}MB.` });
        }
    ...
    const { name, ageYears, ageMonths, weight, species, gender, breed, medicalInfo, owner } =
      req.body;
    ...
    if (!name || !species || !gender || !breed || !owner) {
      return res
        .status(400)
        .json({ message: 'Name, species, gender, breed, and owner are required.' });
    }
    ...
    if (isNaN(numWeight) || numWeight <= 0 || numWeight > 200) {
      return res
        .status(400)
        .json({ message: 'Valid weight (greater than 0, max 200) is required.' });
    }
    // Validate enum fields (species, gender).
    if (!['Dog', 'Cat'].includes(species)) {
      return res
        .status(400)
        .json({ message: 'Invalid species provided. Only Dog or Cat allowed.' });
    }
    ...
    try {
      ...
      if (req.file) {
        ...
        try {
          ...
          return res
            .status(500)
            .json({ message: uploadError.message || 'Failed to upload picture to storage.' });
        }
      }
...
router.put('/:id', (req, res, next) => {
  ...
  upload.single('picture')(req, res, async (err) => {
    ...
    try {
      ...
      if (name !== undefined) {
        if (!name.trim())
          return res.status(400).json({ message: 'Pet name cannot be empty if provided.' });
      ...
      if (ageYears !== undefined) {
        ...
        if (isNaN(numAgeYears) || numAgeYears < 0 || numAgeYears > 50) {
          return res
            .status(400)
            .json({ message: 'Valid age in years (0-50) is required for update.' });
        }
      ...
      if (ageMonths !== undefined) {
        ...
        if (isNaN(numAgeMonths) || numAgeMonths < 0 || numAgeMonths > 11) {
          return res
            .status(400)
            .json({ message: 'Valid age in months (0-11) is required for update.' });
        }
      ...
      if (weight !== undefined) {
        ...
        if (isNaN(numWeight) || numWeight <= 0 || numWeight > 200) {
          return res
            .status(400)
            .json({ message: 'Valid weight (0.1-200) is required for update.' });
        }
      ...
      if (breed !== undefined) {
        if (!breed.trim())
          return res.status(400).json({ message: 'Breed cannot be empty if provided.' });
        updateData.breed = breed.trim();
      }
      if (medicalInfo !== undefined) {
        // medicalInfo can be an empty string.
      ...
      if (req.file) {
        ...
        try {
          ...
          return res
            .status(500)
            .json({ message: uploadError.message || 'Failed to upload new picture.' });
        }
      }
      ...
      if (error.name === 'ValidationError') {
        return res
          .status(400)
          .json({ message: 'Validation error during update.', details: error.errors });
      }
