...
/**
 * Checks for upcoming schedules within a defined time window
 * and sends reminder emails to the respective owners.
...
async function runReminderCheckJob() {
  ...
  console.log(
    `Reminder Window (UTC): [${reminderWindowStartUTC.toISOString()}, ${reminderWindowEndUTC.toISOString()})`,
  );
  ...
  try {
    ...
    if (!rules || rules.length === 0) {
      ...
      for (const occurrence of upcomingOccurrences) {
        ...
        if (ownerEmail) {
          const eventStartTimeLocal = moment(occurrence.start)
            .tz(reminderJobTimezone)
            .format('h:mm a');
          console.log(
            `Attempting to send reminder for "${occurrence.title}" (Starts: ${eventStartTimeLocal}) to ${ownerEmail}`,
          );
          try {
            await sendReminderEmail(ownerEmail, occurrence.title, occurrence.start);
            ...
            try {
              ...
              if (saveError.code === 11000) {
                // Handles potential race conditions.
                console.warn(
                  `Race condition: Reminder ${reminderKey} was likely marked as sent by another process.`,
                );
              } else {
            ...
            console.error(
              `Error sending reminder email for rule ${occurrence.ruleId}, occurrence start ${occurrence.start.toISOString()}:`,
              emailError,
            );
          }
        } else {
          console.warn(
            `Cannot send reminder for rule ${occurrence.ruleId}, occurrence start ${occurrence.start.toISOString()}: Owner email not found.`,
          );
        }
      }
    }
