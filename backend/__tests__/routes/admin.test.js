...
beforeEach(async () => {
  ...
  adminToken = jwt.sign({ id: adminUser._id, role: adminUser.role }, jwtSecret.secret, {
    expiresIn: '1h',
  });
  regularUserToken = jwt.sign({ id: regularUser._id, role: regularUser.role }, jwtSecret.secret, {
    expiresIn: '1h',
  });
});
...
describe('Admin Routes', () => {
  ...
  describe('DELETE /admin/users/:id', () => {
    it('should delete a user and their associated data (pets, schedules)', async () => {
      ...
      expect(response.body.message).toContain(
        'User and associated pets and schedules deleted successfully',
      );
      ...
      const deletedUserActivities = activities.filter(
        (a) => a.type === 'user_deleted' && a.userId.toString() === userIdToDelete.toString(),
      );
      const deletedPetActivities = activities.filter(
        (a) => a.type === 'pet_deleted' && a.userId.toString() === userIdToDelete.toString(),
      );
  ...
  describe('PUT /admin/settings/password', () => {
    ...
    it('should return 400 if new password fails validation (e.g., too short)', async () => {
      ...
      expect(response.body.message).toMatch(
        /Password must be at least 8 characters long|Password validation failed/i,
      );
    });
  });
  ...
  describe('PUT /admin/settings/profile', () => {
    ...
    it('should return 400 if age is invalid (e.g., less than 13)', async () => {
      ...
      expect(response.statusCode).toBe(400);
      expect(response.body.message).toMatch(
        /Age must be a number between 13 and 120|Validation failed/i,
      );
    });
