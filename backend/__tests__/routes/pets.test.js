...
const mockUploadUrl =
  'https://mock-bucket.s3.mock-region.amazonaws.com/mock-uploads/pets/mock-pet-pic.jpg';
...
describe('Pet Routes', () => {
  ...
  describe('POST /pets/add', () => {
    ...
    it('should return 400 for missing required fields', async () => {
      ...
      expect(response.body.message).toMatch(
        /Name, species, gender, breed, and owner are required/i,
      );
    });
  ...
  describe('DELETE /pets/:id', () => {
    ...
    it('should delete a pet without pictures', async () => {
      ...
      const petNoPic = await Pet.create({
        name: 'NoPic',
        ageYears: 1,
        ageMonths: 0,
        weight: 10,
  ...
  describe('PUT /pets/:id', () => {
    ...
    it('should update pet details successfully', async () => {
      ...
      const response = await request(app).put(`/pets/${petToUpdate._id}`).send(updates);
    ...
    it('should return 404 if pet ID does not exist', async () => {
      ...
      const response = await request(app).put(`/pets/${invalidId}`).send(updates);
