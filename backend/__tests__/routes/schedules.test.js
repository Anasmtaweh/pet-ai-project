...
describe('Schedule Routes', () => {
  ...
  describe('POST /schedules/add', () => {
    ...
    it('should add a new schedule successfully', async () => {
      const response = await request(app).post('/schedules/add').send(validScheduleData);
    ...
    it('should add a repeating schedule successfully', async () => {
      ...
      const response = await request(app).post('/schedules/add').send(repeatingData);
    ...
    it('should return 400 if required fields are missing', async () => {
      ...
      const response = await request(app).post('/schedules/add').send(invalidData);

      // Expect 400 because the manual validation check in the route catches it
      expect(response.statusCode).toBe(400);
      expect(response.body.message).toBe(
        'Missing required fields (title, start, end, type, owner)',
      );
    });
  ...
  describe('PUT /schedules/:id', () => {
    ...
    beforeEach(async () => {
      scheduleToUpdate = await Schedule.create({
        title: 'Initial Title',
        start: new Date(),
        end: new Date(),
        type: 'play',
    ...
    it('should update a schedule successfully', async () => {
      ...
      const response = await request(app).put(`/schedules/${scheduleToUpdate._id}`).send(updates);
    ...
    it('should return 404 if schedule ID does not exist', async () => {
      ...
      const response = await request(app).put(`/schedules/${invalidId}`).send(updates);
  ...
  describe('DELETE /schedules/:id', () => {
    ...
    beforeEach(async () => {
      scheduleToDelete = await Schedule.create({
        title: 'To Delete',
        start: new Date(),
        end: new Date(),
        type: 'vet',
    ...
    it('should delete a schedule successfully', async () => {
      const response = await request(app).delete(`/schedules/${scheduleToDelete._id}`);
      ...
      // Verify Recent Activity
      const activity = await RecentActivity.findOne({
        scheduleId: scheduleToDelete._id,
    ...
    it('should return 404 if schedule ID does not exist', async () => {
      const invalidId = new mongoose.Types.ObjectId().toString();
      const response = await request(app).delete(`/schedules/${invalidId}`);
  ...
  describe('POST /schedules/:id/exception', () => {
    ...
    it('should return 400 if occurrenceDate is missing', async () => {
      const response = await request(app).post(`/schedules/${scheduleRule._id}/exception`).send({}); // Missing occurrenceDate in payload.
