...
describe('Mailer Utilities', () => {
  ...
  describe('sendPasswordResetEmail', () => {
    ...
    it('should throw error if sendMail fails', async () => {
      ...
      await expect(mailer.sendPasswordResetEmail(testEmail, testToken)).rejects.toThrow(
    ...
    it('should log error and not send if transporter is not configured', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          `Cannot send email ("MISHTIKA Password Reset Request" to ${testEmail}): Email transporter is not configured.`,
        ),
      );
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'WARNING: Email configuration (EMAIL_USER, EMAIL_PASS) is missing.',
        ),
      );
    });
  });
  ...
  describe('sendReminderEmail', () => {
    ...
    it('should call sendMail with correct options and formatted time for reminder', async () => {
      ...
      const expectedFormattedTime = moment(testStartTimeUTC)
        .tz('Asia/Beirut')
    ...
    it('should throw error if sendMail fails for reminder', async () => {
      ...
      await expect(
        mailer.sendReminderEmail(testEmail, testTitle, testStartTimeUTC),
    ...
    it('should log error and not send reminder if transporter is not configured', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          `Cannot send email ("Reminder: ${testTitle}" to ${testEmail}): Email transporter is not configured.`,
        ),
      );
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining(
